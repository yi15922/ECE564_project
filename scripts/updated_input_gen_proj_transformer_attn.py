...
"""
def convert_matrix_to_dat(matrix, address, row_size, col_size, file_str, data_format):
    max_value = (1 << 31) - 1  # Maximum value for 32-bit signed integer
    min_value = -(1 << 31)     # Minimum value for 32-bit signed integer
    for i, row in enumerate(matrix):
        print(f"i = {i}, row = {row}")
        for j, element in enumerate(row):
            print(f"j = {j}, col = {element}")
            print(f"address = {(address + (i * col_size) + j)}")
            ...
            # Convert element to 32-bit two's complement hex
            if data_format == "INTEGER":
                # Directly cast to integer and ensure 32-bit two's complement
                value = int(element) & 0xFFFFFFFF
            elif data_format == "FLOATING_POINT":
                # Convert to fixed-point (assuming 8 fractional bits)
                fixed_value = round(element * (1 << 8))  # 8 fractional bits
                value = fixed_value & 0xFFFFFFFF

#            if value > max_value or value < min_value:
#                print(f"Value {value} is out of range for 32-bit fixed-point representation.")
#                value = max(min(fixed_value, max_value), min_value)

            # Add formatted output to the file string
            file_str += " @{:08X} {:08X}\n".format(address + (i * col_size) + j, value)
...
def main(argv):
  ...
  if data_format=="INTEGER":
    input_matrix = np.random.randint(-10, 10,size=[input_row_size,input_col_size])    
  elif data_format=="FLOATING_POINT":
    input_matrix = np.single(np.random.uniform(1,-1,size=[input_row_size,input_col_size]))
  
  if data_format=="INTEGER":
    weight_q_matrix = np.random.randint(-10, 10,size=[weight_row_size,weight_col_size])
    weight_k_matrix = np.random.randint(-10, 10,size=[weight_row_size,weight_col_size])
    weight_v_matrix = np.random.randint(-10, 10,size=[weight_row_size,weight_col_size])
  elif data_format == "FLOATING_POINT":
